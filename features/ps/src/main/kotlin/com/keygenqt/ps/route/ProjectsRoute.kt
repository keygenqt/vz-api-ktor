...
fun Route.projectsRoute() {
    ...
    route("/projects") {
        ...
        get("/{id}") {
            val id = call.parameters["id"]?.toIntOrNull() ?: throw AppException.Error404("Must provide id number")
            call.respond(service.getById(id) ?: throw AppException.Error404("Model not found"))
        }
    }
}
